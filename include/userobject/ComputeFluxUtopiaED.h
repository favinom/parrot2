...
class ComputeFluxUtopiaED;

template <>
InputParameters validParams<ComputeFluxUtopiaED>();

class ComputeFluxUtopiaED : public GeneralUserObject
{
public:
  static InputParameters validParams();

  ComputeFluxUtopiaED(const InputParameters & params);
  ...
  std::vector<dof_id_type>  _dirIdsM;
  std::vector<dof_id_type>  _dirIdsF;
  std::vector<int> _vector_p;
  std::vector<Real> _vector_value_m;
  std::vector<Real> _vector_value_f;
  std::string _multiapp_name;

  void LagrangeMultiplier(const utopia::UVector lambda);
