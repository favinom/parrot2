...
registerMooseObject("parrot2App", ComputeFluxID);

defineLegacyParams(ComputeFluxID);

InputParameters ComputeFluxID::validParams()
{
...
ComputeFluxID::ComputeFluxID(const InputParameters & parameters) :
...
void ComputeFluxID::init()
{
  std::cout<<"ComputeFluxID::init() start\n";
  ...
  _linearImplicitSystemM[0].add_matrix(_mat_domain_name_2.c_str());
  ...
  _linearImplicitSystemM[0].add_matrix(_mat_binterp_name_2.c_str());
  ...
  _linearImplicitSystemM[0].add_vector(_vec_flux_name_2.c_str());

  _linearImplicitSystemM[0].add_vector(_vec_rhs_name_0.c_str());
  _linearImplicitSystemM[0].add_vector(_vec_rhs_name_1.c_str());
  _linearImplicitSystemM[0].add_vector(_vec_rhs_name_2.c_str());
  ...
  _linearImplicitSystemM[0].add_vector(_vec_boundary_name_2.c_str());
  ...
  std::cout<<"ComputeFluxID::init() stop\n";
}

void ComputeFluxID::setDirichlet()
{
  std::cout<<"ComputeFluxID::setDirichlet() start\n";
  ...
  std::cout<<"ComputeFluxID::setDirichlet() stop\n";
}

void ComputeFluxID::solve()
{
  std::cout<<"ComputeFluxID::solve() start\n";
  ...
  SparseMatrix<Number> & A2 = _linearImplicitSystemM[0].get_matrix(_mat_domain_name_2.c_str());
  ...
  SparseMatrix<Number> & I2 = _linearImplicitSystemM[0].get_matrix(_mat_binterp_name_2.c_str());
  ...
  NumericVector<Number> & f2 = _linearImplicitSystemM[0].get_vector(_vec_flux_name_2.c_str());
  ...
  NumericVector<Number> & b2 = _linearImplicitSystemM[0].get_vector(_vec_rhs_name_2.c_str());
  ...
  NumericVector<Number> & i2 = _linearImplicitSystemM[0].get_vector(_vec_boundary_name_2.c_str());
...

 {
  ...
  MooseVariableFEBase  & _sol_aux = _fe_problem.getVariable(0, "aux_sol", Moose::VarKindType::VAR_ANY, Moose::VarFieldType::VAR_FIELD_STANDARD);
  ...
  MooseVariableFEBase  & _sol_main = _fe_problem.getVariable(0, "pressure", Moose::VarKindType::VAR_ANY, Moose::VarFieldType::VAR_FIELD_STANDARD);
          ...
          for (const auto & node : _fe_problem.es().get_mesh().local_node_ptr_range())
          {
            ...
            for (unsigned int comp = 0; comp < node->n_comp(main_sys.number(), _sol_main.number()); comp++)
              
              {
                ...
                //Real value_pressure=0;

                //aux_solution->get(to_index_1, value_pressure);
  ...
  if(_has_block_2_id) std::cout<<f2.sum()<<std::endl;

  std::cout<<i0.sum()<<std::endl;
  std::cout<<i1.sum()<<std::endl;
  if(_has_block_2_id) std::cout<<i2.sum()<<std::endl;
  ...
  std::cout<<"ComputeFluxID::solve() stop\n";
}

    void ComputeFluxID::write()
    {
      std::cout<<"ComputeFluxID::write() start\n";
      ...
      std::cout<<"ComputeFluxID::write() stop\n";
    }

    void ComputeFluxID::del()
    {
      std::cout<<"ComputeFluxID::del() start\n";

      // //_equationSystemsP[0].delete_system(_sys_name.c_str());
      // _equationSystemsP[0].clear();

      std::cout<<"ComputeFluxID::del() stop\n";  
    }

    void ComputeFluxID::initialize()
    {
...
void ComputeFluxID::assemble()
{
  std::cout << "ComputeFluxID::assemble start\n";
  ...
  std::cout<<"_dim_F==>"<<_dim<<std::endl;

  std::cout<<_qrule->get_order()<<std::endl;

  std::cout<<_qrule->type()<<std::endl;
 ...
 int count_2=0;
  ...
  for (auto side : elem->side_index_range())
  {
    ...
    if (otherElem != nullptr)
    {
      ...
      if (elemId!=otherID)
      {
        ...
        {
          i2.add_vector (rel  , dof_indices_B  );
          for (int i=0; i<kel.m();++i)
            for (int j=0; j<kel.n();++j)
              I2.set(dof_indices_B.at(i),dof_indices_B.at(j),kel(i,j));
        }
        else
        {
  ...
  //std::cout<<"count_2=>"<<_block_2_id<<std::endl;
  ...
  else if (_has_block_2_id == true && elemId ==_block_2_id)
  {
    ...
    b2.add_vector (re  , dof_indices  );
  }
  else
  {
    //std::cout<<"error"<<elemId<<std::endl;
    mooseError("elemId does not belong to any block");
  }
  ...
  std::cout << "ComputeFluxID::assemble stop\n";
}

Real
ComputeFluxID::ComputeMaterialProprties(const Elem *elem)
{
